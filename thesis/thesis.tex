...
% 5 {} token are mandatory
  ...
  % {} leave it empty if necessary  
   {In this thesis, we study the interesting chemistry of a newly found hot corino CARMA-7 in Serpen South region with a set of ALMA observation data. Chemical studies on hot corinos are relatively limited (15 sources detected so far). Studying chemistry traits of a hot corino source will also help suggest any potential motion / structural information. }
  % aims heading (mandatory)
   {We aim to identify as many molecular lines as possible from observation data with a set of tools including XCLASS, ADMIT, CASA and STATCONT. We use these tools to preprocess and then analyze our data cubes.}
  % methods heading (mandatory)
   {We used STATCONT to preprocess data cubes in order to determine continuum, followed by LineIdentification task in ADMIT (integrated with CASA) and (possibly) XCLASS. }
  % results heading (mandatory)
   {The CARMA-7 source is chemically rich and possibly a source of vibrant motions.}
...
\section{Observation}
   ...
   \begin{figure*}
    ...
    \caption{Promising lines in spectral window 0 ($vlsr = 8.0$)}
 ...
 \tiny
    ...
    \hline\hline       
    Molecule$^{1}$ & Name &Transition & Frequency$^{2}$ & $E_{u}^{3}$ & Intensity$^{4}$ & Velocity$^{5}$ & $v_{lsr}$ & Peakrms\\ 
    \hline    
